...
describe('icon assets', () => {
  it('uses the Palutena icon artwork for browser and PWA metadata', () => {
    ...
    expect(icon).toContain('image-rendering="pixelated"')
    expect(maskableIcon).toContain('image-rendering="pixelated"')
    expect(icon).toContain('href="data:image/png;base64,')
    expect(maskableIcon).toContain('href="data:image/png;base64,')
  })
})
