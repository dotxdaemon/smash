...
- Follow-up: Sean asked to fix everything and land it on `main`. Removed the now-orphaned `public/favicon`-era `public/vite.svg` (referenced nowhere in code), re-ran the full suite (still 36 passed, typecheck/lint/build clean), and fast-forwarded `main` (which had been left stale at `aeed2f3`, behind the whole neon-noir rebuild) up to the branch tip so `main` finally reflects the shipped app plus the favicon fix.
- Request: Sean said the app "needs to be more useful", then clarified mid-pass: no random stuff that makes it unusable, and make the UX prettier.
- What I tried last: The tracker logged sets and showed records, but the Seraph Notes sat disconnected from the loss tags, saving a set gave zero feedback, stats never showed habit trends, and the focus panel was a bare left-border strip.
- Attempt: Kept the additions lean and closed the coaching loop instead of adding surfaces. Tagged each Seraph Note with related loss tags (`relatedTags` on `SERAPH_NOTES`, typed instead of `as const` so `.includes` typechecks), added `getDrillsForTag` + drills on `TrainingFocus` (top 2), `getLossHabits` (totals + last-10 window), and `getCurrentStreak`. The log/matchup focus now links its drills straight to the Notes tab; saving a set shows a "Win vs X saved." toast with Undo that deliberately does NOT steal focus (`focusAction: false` on ToastData) so rapid logging keeps the opponent field focused; delete undo behavior is unchanged via one `UndoableAction` union. Stats leads with a top-3 "Loss habits" card; the scoreboard shows a W/L streak only at 2+. Polish: the focus panel became a cyan glass coach card, Loss habits its vermilion twin, and the Save button got a real primary treatment with a soft glow.
- Error after I tried: None functional this pass; the only friction was the editor state tracker repeatedly demanding re-reads before edits.
- Result: Log screen now reads habit → rule → drill with tappable drills, saves confirm and can be undone, Stats answers "what keeps beating me and is it recent", and the header shows the current run. Verified live via CDP: drill click lands on Notes, "Win vs Wolf saved." toast (canonicalized from `wolf`), focus stays in the form, Undo removes the row from localStorage, and the header updates 4-4→5-4 / 50%→56% / W2→W3 on save.
- Verification: `npm test` (47 passed, +11), `npm run typecheck`, `npm run lint`, `npm run build`, and `npm run build -- --base=/smash/` all passed; desktop 1280x900 and mobile 390x844 screenshots captured for the log coach card, stats habits card, and the save toast, with no console errors.

## 2026-07-04

- Request: Fix the tracker UI because it looks bad and is confusing — make it much more UX friendly.
- What I tried last: The neon-noir rebuild was refined into a hairline/editorial system (82cb492) where tabs, the history filter, the result toggle, loss-tag chips, text inputs, and clickable opponent names all rendered as near-identical faint underlined text, saving gave no feedback, and opening a matchup from Stats silently switched the active tab to History.
- Attempt: Renamed the visual system to night-console and gave every interactive control visible chrome while content stayed in open lists: boxed inputs with focus rings, a contained Win/Loss segmented toggle, ✓-marked pill loss tags under a "What went wrong?" legend, a solid cyan Save/CTA treatment, a contained history filter, button-shaped back/export actions, chevroned opponent links with row hover, W/L letter badges instead of dots, and thicker two-tone win-rate bars. Kept the focus panel title stable with the driving tag as a chip, added a "Set logged — vs X" confirmation toast (toast action button now optional), kept matchup drill-ins on their origin tab with a matching back label, renamed the Notes tab to Reference, shrank the header so the log form fits above the fold, brightened muted/faint text tokens, and removed the rain animation, vertical 戦績 sign, kanji empty-state glyphs, and frame backdrop blur.
- Error after I tried: None new — the six failing-first assertions (night-console marker, Reference tab, no decorative glyph chrome, stable focus kicker with focus-tag chip, result-badge letters, action-optional toast) failed as expected before the change and passed after it.
- Result: Controls read as controls at a glance, saves confirm visibly in the live region, Stats drill-ins keep the Stats tab active with "← Back to stats", and the whole log form is visible on the first screen. The workflow, `smash-tracker` storage contract, and a11y hooks (tablist, tabpanel, live region, focus management, Escape-to-close) are unchanged.
- Verification: `npm test` (40 passed), `npm run lint`, `npm run typecheck`, `npm run build`, and `npm run build -- --base=/smash/` all exited 0. Browser-verified at desktop and 390x844: save toast plus record update, loss chips with checkmarks, history search and filter, delete then Undo restoring 11/11 rows, matchup origin from both History and Stats, the Reference tab, empty states with a solid CTA, 16px inputs (no iOS focus zoom), and no horizontal overflow or console errors.
- Push reconciliation: `origin/main` had moved ahead with a parallel session's favicon fix and its coaching-loop pass (68f0b17: drills linked from the focus panel to the Seraph Notes, a save toast with undo that keeps focus in the form, a Loss habits card on Stats, and a scoreboard streak). The two passes were complementary — theirs functional, this one affordance and navigation — so both were merged: all of their logic, tests, and features landed intact; this pass's night-console system, top-level matchup drill-in with origin-aware back label, and solid-fill primary button govern presentation. Their drill links and Loss habits card were restyled from the hairline idiom into night-console chrome (pill drill buttons, the habits card as the vermilion twin of the cyan focus panel), their save toast (result + canonicalized opponent + Undo, `focusAction: false`) replaced this pass's plainer message, and the merged `LogForm` keeps the stable focus title with the tag chip beside their drills row.
- Verification after merge: `npm test` (52 passed), `npm run lint`, `npm run typecheck`, `npm run build`, and `npm run build -- --base=/smash/` all exited 0 (one fix: the new `LogForm` tests needed the now-required `onOpenNotes` prop). Browser-verified merged behavior at desktop and 390x844: drill pill jumps to Reference, "Win vs Wolf saved." with focus kept in the opponent field and Undo removing the row (12→13→12), the L2 streak figure, the Loss habits card above the opponent bars, and the matchup drill-in from Stats keeping its origin tab.
- Request: Update the Notes tab with the latest notes from the Notion `Smash!` page, especially `Playing against other people` and the `Smash Fundamentals` reminder, while preserving the product structure.
- What I tried last: The Reference tab only rendered the earlier Seraph Notes dataset, so the newer Notion page content had no in-app data source or render path.
- Attempt: Fetched the exact `Smash!` Notion page, added a failing render test for every current source line, then placed the two source-backed sections above the existing Seraph drills using the current cardless reference styles.
- Error after I tried: The targeted test failed before implementation because `Playing against other people` was absent from the rendered Reference view.
- Result: The Reference tab now starts with the three people notes and the Smash Fundamentals reminder verbatim, with no changes to navigation, storage, the service worker, dependencies, or CSS.
- Verification: `npm test -- src/components/ReferenceView.test.tsx`, `npm test` (53 passed), `npm run lint`, `npm run typecheck`, `npm run build`, and `npm run build -- --base=/smash/` exited 0. Browser verification at 390x844 confirmed both sections, no horizontal overflow, and a clean screenshot.
- Request: Put the Notion `Smash!` notes on a separate page instead of combining them with Seraph Notes.
- What I tried last: The prior pass rendered the Notion sections directly inside `ReferenceView`, so both note collections necessarily appeared in one tab panel.
- Attempt: Added failing tests for distinct `Notes` and `Reference` tabs, a Notion-only notes view, and a Seraph-only reference view; then moved the Notion content into its own data file and top-level page while keeping Seraph drill links pointed at Reference.
- Error after I tried: The first targeted run failed because `NotesView` did not exist, the app had no separate Reference tab ID, and `ReferenceView` still contained both Notion sections. The first 390x844 proof also showed `Log set` wrapping because the fifth tab exceeded the mobile navigation width.
- Result: Notes and Reference are separate top-level pages, each contains only its intended source, and a mobile-only tab padding/nowrap adjustment keeps all five labels on one line without horizontal overflow.
- Verification: `npm test -- src/App.test.tsx src/components/ReferenceView.test.tsx src/components/NotesView.test.tsx`, `npm test` (53 passed), `npm run lint`, `npm run typecheck`, `npm run build`, and `npm run build -- --base=/smash/` exited 0. Browser verification at 390x844 confirmed separate panel IDs and content, 316px navigation width with 316px scroll width, 390px document width, and no console errors.
- Request: Redo the UI with a completely fresh direction, without taking cues from previous versions.
- What I tried last: The tracker used a centered dark night-console frame, atmospheric neon background, teal controls, and a recent chrome-focused redesign that still kept the same visual identity.
- Attempt: Added a failing shell-contract test, removed the decorative backdrop, and replaced the shared visual system with a bright paper-scorebook direction: black editorial masthead, electric-blue live score block, vertical desktop navigation, compact mobile tabs, open paper workspace, crisp rules, and square print-like controls. Preserved all workflows, accessibility hooks, storage behavior, service worker behavior, and deployment paths.
- Error after I tried: The new shell-contract test first failed because `night-console`, `compact-workspace`, and `neon-backdrop` were still rendered; it passed after the shell and stylesheet change. The in-app browser DOM snapshot method was unavailable, so bounded DOM reads and unique accessible locators were used for browser acceptance instead.
- Result: The app now has a distinct light editorial identity rather than another iteration of the neon UI. The real save flow retained opponent-field focus, tagged losses appeared in History, loss habits and opponent rows appeared in Stats, and the final 390x844 layout had no horizontal overflow with 16px inputs.
- Verification: `npm test` (53 passed), `npm run lint`, `npm run typecheck`, `npm run build`, and `npm run build -- --base=/smash/` exited 0. Browser-verified at 1280x720 and 390x844 with clean desktop/mobile screenshots and no console warnings or errors.
- Follow-up request: Sean said the paper-scorebook redesign looked "100x worse" and selected Notion as the visual direction for the correction.
- What I tried last: The failed pass used a 288px mobile masthead, saturated score strip, oversized condensed title, thick borders, and a poster-like desktop frame that made navigation and branding compete with the logging task.
- Attempt: Verified the exact current Notion workspace model against Notion's official workspace and sidebar guides, added a failing Notion workspace shell test, grouped identity/record/navigation into one neutral sidebar, and restyled the task area as a quiet white editor with system typography, subtle row selection, low-chrome inputs, and blue limited to actions and focus.
- Error after I tried: The visual-contract test first failed because the app still rendered `paper-scorebook`; browser inspection initially targeted the Notion reference tab instead of localhost, then succeeded after explicitly navigating the preview tab back to the local app.
- Result: The mobile workspace now starts at 126px instead of 342px, desktop uses a 240px neutral sidebar and white editor, input text remains 16px, and the save/undo, History, Stats, Notes, and Reference flows all work without horizontal overflow or console errors.
- Verification: `npm test` (53 passed), `npm run lint`, `npm run typecheck`, `npm run build`, and `npm run build -- --base=/smash/` exited 0. Fresh 1280x720 and 390x844 screenshots were captured alongside the official Notion reference and the failed mobile before state.
- Standing preference: Sean wants completed app changes committed to `main`, pushed to `origin/main`, and followed through to a successful production deployment without needing to ask each time. Treat live visibility as part of done unless Sean explicitly requests local-only work or review without publication.
- Verification: At the time this preference was recorded, `main` matched `origin/main`, Vercel reported commit `8a0ddc7` deployed successfully, and `npm test` (53 passed), `npm run lint`, and `npm run typecheck` exited 0.

## 2026-07-09

- Request: Redo the UI because the Notion-directed correction still looked horrendous.
- What I tried last: The previous correction replaced the failed paper-scorebook with a pale Notion clone, but the sidebar, record, tabs, coaching prompt, fields, and actions all used nearly the same quiet gray weight, leaving no focal point or product identity.
- Attempt: Added a failing shell-contract test for a `match-ledger` visual system and `rail-dock` layout, then replaced the shared presentation with a dark, compact training ledger: persistent desktop utility rail, safe-area-aware mobile dock, mint action/state accent, semantic red, clearer scoreboard, stronger coaching hierarchy, and denser mobile logging rhythm. Kept the component structure, workflows, accessibility hooks, localStorage shape, and service worker untouched.
- Error after I tried: The first 390x844 acceptance check found the fixed mobile dock overlapping the Save action by 54px. Tightening the form reduced that to 7px, but the textarea stayed 97px tall because its `rows=3` intrinsic height overrode the smaller `min-height`; an explicit mobile height fixed the root cause and left 10px of clearance in the initial viewport.
